...
def launch_master_thread(n, ids, root, conn_matrix):
    ...
    print 'In master thread. Launching {0} threads.'.format(n)
    initial_config = {}
    q = Queue.Queue() # initially empty queue
    for pid in ids:
    ...
    r=1
    while True:
        print '********** Master thread broadcast --> Begin round {0} ***********'.format(r)
        if r==1:
            config = initial_config
            latest_q = q
            print '|| BFS LEVEL: {0}'.format(root)
        r = r+1
    ...
    print('**********************************************')
    for c in config:
        print '{0} : Parent: {1} Children: {2} Marked: {3}'.format(c, config[c]["parent"], config[c]["children"], config[c]["marked"])
